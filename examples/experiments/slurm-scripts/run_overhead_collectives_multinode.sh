...
#SBATCH -e err_overhead_collectives_multinode-%J
#SBATCH -o out_overhead_collectives_multinode-%J
...
EXE="${NCCL_PATH}/examples/experiments/01_collectives_overhead/collectives_overhead_mpi"

# Check if executable exists
if [ ! -f "$EXE" ]; then
    echo "ERROR: Executable not found. Please run 'make' in examples/experiments/01_collectives_overhead"
    exit 1
fi
