#!/bin/bash
# NCCL Profiler overhead: collectives single-node using NVIDIA nccl-tests.
# Uses all_reduce_perf. Reproducible with upstream nccl-tests.
# nccl-tests: https://github.com/NVIDIA/nccl-tests

#SBATCH --job-name=nccl_tests_coll
#SBATCH --account=p_lv_nccl

#SBATCH --time=00:15:00

#SBATCH -e err_nccl_tests_coll-%J
#SBATCH -o out_nccl_tests_coll-%J
...
echo "=========================================="
echo "NCCL Profiler Overhead - Collectives (nccl-tests, single-node)"
echo "=========================================="
echo "Job ID: $SLURM_JOB_ID"
echo "Nodes: $SLURM_NODELIST"
echo "nccl-tests: $NCCL_TESTS_HOME"
echo ""
...
if [ ! -f "$EMPTY_PLUGIN" ]; then
    ...
    echo "Build it: cd ${NCCL_PATH}/ext-profiler/empty-profiler && make"
fi

RESULTS_DIR="nccl_tests_results_coll_${SLURM_JOB_ID}"
mkdir -p "$RESULTS_DIR"

# Same order of magnitude as report: 1M iters, 100 warmup; small sizes for overhead
NITER=1000000
WARMUP=100
OPTS="-b 8 -e 128M -f 2 -g 4 -n $NITER -w $WARMUP"
