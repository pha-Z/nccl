...
#SBATCH --job-name=overhead_comm_ops_multinode
#SBATCH --account=p_lv_nccl

#SBATCH --time=00:10:00

#SBATCH -e err_overhead_comm_ops_multinode-%J
#SBATCH -o out_overhead_comm_ops_multinode-%J
...
echo "=========================================="
echo "NCCL Profiler Overhead - Communicator Operations (Multi-Node)"
echo "=========================================="
...
EXE="${NCCL_PATH}/examples/experiments/03_comm_ops_overhead/comm_ops_overhead_mpi"

# Check if executable exists
if [ ! -f "$EXE" ]; then
    echo "ERROR: Executable not found. Please run 'make' in examples/experiments/03_comm_ops_overhead"
    exit 1
fi
