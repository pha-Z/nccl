...
\documentclass[english,hauptseminar,hyperref,bibnum]{zihpub}
...
% Short ``page ref'' style footnotes for NCCL repo code (base URL cited once in References)
\newcommand{\ncclrepobase}{https://github.com/NVIDIA/nccl/blob/v2.28.7-1}
\newcommand{\ncclref}[1]{\footnote{\href{\ncclrepobase/#1}{\texttt{\detokenize{#1}}}.}}
...
Next, considerations for developers of the Profiler API and its potential integration with \mbox{Score-P\xspace} is discussed. 
...
with the code published on GitHub\cite{nccl-repo}.
The release of NCCL 2.0 in 2017 brought support for NVLink, however it was initially only available as pre-built binaries.
With the release of 2.3 in 2018, NCCL returned to being fully open source on GitHub.
...
\begin{itemize}
\item \textbf{MPI}: Communication is process-based. A rank corresponds to a single host process within a communicator.
\item \textbf{NCCL}: Communication is GPU-based, with CPU threads handling orchestration. 
A rank corresponds to a GPU device within a communicator, 
where the mapping from ranks to devices is bijective. 
A single CPU thread can manage multiple ranks (i.e. multiple devices) in a communicator using the functions \texttt{ncclGroupStart} and \texttt{ncclGroupEnd}. 
A CPU thread can also manage multiple ranks from \emph{different} communicators (i.e. same device alloted by ranks from different communicators through communicator creation with \texttt{ncclCommSplit} or \texttt{ncclCommShrink}),
so the mapping from ranks to threads is surjective.
\end{itemize}

\subsection{Relevant NCCL internals}
For certain behaviors of the Profiler API, it is helpful to first understand what NCCL does internally when an application calls the NCCL User API.
...
This thread lazily starts another thread called \texttt{ProxyProgress}\ncclref{src/proxy.cc}, 
...
\end{call}
\end{sequencediagram}
\caption{Thread creation: User API $\to$ NCCL internal init $\to$ create ProxyService $\to$ create Proxy\-Progress.}
\label{fig:thread-creation}
\end{figure}

\texttt{if}-guards ensure that these threads are created once per \texttt{ncclSharedResources}\ncclref{src/include/comm.h}.
\iffalse
...
NCCL internally decides what network operations to perform and calls \texttt{ncclProxyPost} to post them to a \texttt{proxyOpsPool}.
...
\end{sequencediagram}%
\caption{Flow from User API to ncclProxyPost}
\label{fig:proxy-post}
\end{figure}

The \texttt{ProxyProgress} thread reads from this pool when calling \texttt{ncclProxyGetPostedOps} and progresses the ops.
...
\end{sdblock}
\end{sequencediagram}%
\caption{The ProxyThread runs in a simple progressing loop: progress ops, then get posted ops (or wait). }
...
Whenever the user application calls the Collectives or P2P API (e.g. \texttt{ncclAllReduce}) with this communicator,
...
Each time a NCCL communicator is created, \texttt{ncclProfilerPluginLoad}\ncclref{src/plugin/profiler.cc} is called, 
where NCCL looks for a shared library that represents the profiler plugin by checking an environment variable. 
It then calls \texttt{dlopen}\ncclref{src/plugin/plugin_open.cc} and \texttt{dlsym} to load the library immediately with local symbol visibility:
\begin{lstlisting}[language=C]
...
A \texttt{profilerPluginRefCount} keeps track of the number of calls to this procedure to ensure correct unloading during finalization (Fig.~\ref{fig:profiler-init}). 
The NCCL documentation\cite{nccl-docs} also details further the loading behavior:
...
The plugin loading mechanism expects the variable name to follow the naming convention\\ 
...
NCCL features a fallback mechanism to load older versions.
However one instance is known, where a profiler plugin being developed against the NCCL release 2.25.1 with Profiler API version 2, was unable to run with the latest NCCL release\cite{ncclsee}.
Around this point in time, the NCCL repository has undergone a refactor related to the profiler plugin.
...
\caption{User API $\to$ NCCL communicator init $\to$ load profiler plugin and call profiler->init().}
\label{fig:profiler-init}
\end{figure}

\subsection{Profiler API}

The plugin must implement a profiler API specified by NCCL by exposing a struct\ncclref{src/include/plugin/profiler/profiler_v5.h}. 
...
This release happened well after the begin of the study, so the focus will remain on version 5.
Version 6 introduced additional profiler features for Copy-Engine based collective operations, 
otherwise version 6 and version 5 remain the same.

Five functions must be implemented for the API. 
Internally NCCL wraps all calls to the profiler API in custom functions which are all neatly declared in a single file\ncclref{src/include/profiler.h}.

NCCL invokes the profiler API in different code regions to capture start and stop of NCCL groups, collectives, P2P, proxy, kernel and network activity. 
...
This may happen multiple times for the same profiler instance, 
if multiple GPU ranks are managed in the same process 
or if further communicators are created on that process.
...
The plugin developer should set \texttt{int* eActivationMask} to a bitmask, 
indicating which event types the profiler wants to track\ncclref{src/include/plugin/nccl_profiler.h}:
\begin{lstlisting}[language=C]
enum {
...
\end{lstlisting}
The default value is 0, which means no events are tracked by the profiler. 
...
\texttt{eDescr} describes the started event\ncclref{src/include/plugin/profiler/profiler_v5.h}. 
...
All User API calls to Collective or Point-to-Point operations will start a Group API event.
...
It can be thought of as an event hierarchy\ncclref{ext-profiler/README.md} with several depth levels:
\begin{quote}
\begin{spacing}{0.75}
\begin{lstlisting}[style=ascii]
Group API event
...
Internally, some Collectives (e.g. ncclAlltoAll) are implemented as multiple point-to-point operations, triggering many P2pApi and P2p events.
For application settings utilizing ncclGroupStart/End, further examples and visualizations are provided in \autoref{sec:code-examples-visualizations}.
}
...
\caption{progressOps emits ProxyStep/KernelCh/NetPlugin events. getPostedOps emits ProxyOp events. Several events ProxyCtrl are also emitted.}
\label{fig:proxy-event-emission}
\end{figure}

\texttt{op->progress()} progresses transport specific ops. 
This is implemented as a function pointer type\ncclref{src/include/proxy.h}.
...
for different transport methods\ncclref{src/transport/net.cc}\ncclref{src/transport/coll_net.cc}\ncclref{src/transport/p2p.cc}\ncclref{src/transport/shm.cc}.
...
\texttt{recordEventState} is called in the same functions that call \texttt{startEvent}, while always being called after \texttt{startEvent}.
...
\caption{Flow from User API call $\to$ finalize and plugin unload.}
...
\section{Code examples and visualizations}\label{sec:code-examples-visualizations}
...
to the event.
Additionally, blue dashed lines indicate the relationship between events according to the \texttt{parentObj} field
...
Then, appended ops start progressing triggering \texttt{ProxyOps} events, 
...
\texttt{ProxyStep} events are emitted in environments requiring cross node communication. 
If this type of communication is not required, then \texttt{ProxyStep} events will not be emitted either.

\begin{figure}[H]
  \centering
\includegraphics[width=\textwidth]{"images/multi_gpu_single_thread/finalize alt (3).png"}
    ...
    ProxyStep events have been omitted for visual clarity, see Fig.~\ref{fig:multi-gpu-single-thread-show-proxystep} for a depiction.}
  \label{fig:multi-gpu-single-thread-allreduce}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"images/multi_gpu_single_thread/show proxystep alt (3).png"}
  \caption{One device per thread: In Fig.~\ref{fig:multi-gpu-single-thread-allreduce} ProxyStep events have been omitted for visual clarity. 
  However, in multinode settings, many additional profiler API calls for proxyStep events happen, 
  informing about the low level network steps in their event details via recordEventState (indicated as red circles above each of the event bars).
  The blue dashed lines indicate the parentObj of each proxyStep event, 
  which are the above proxyOp events.}
  \label{fig:multi-gpu-single-thread-show-proxystep}
\end{figure}

\subsection{Multiple Devices per Thread (ncclGroup)}\label{sec:multiple-devices-per-thread-ncclgroup}

In this example\ncclref{examples/03_collectives/01_allreduce/}, one NCCL thread manages all GPUs on the same node. 
...
Otherwise all other events are processed as usual and are called their usual amount of times irrespective of \texttt{ncclGroup} (Fig.~\ref{fig:single-gpu-single-process-ncclgroup-finalize}).
This behavior also holds true within a process. 
It also holds when grouping (single) collectives for different communicators.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"images/ncclgroup 03_communicators_01_allreduce/init (3).png"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"images/ncclgroup stress test/ncclgroup commA 1 collX + commA 1 collY/phase 2.5, 1st iter (4).png"}
...
The applications \texttt{sendrecv\_perf} and \texttt{all\_reduce\_perf} were launched with following test parameters:
message size \SI{64}{B}, \num{1000000} iterations per size, 100 warmup iterations. 
...
All experiments were carried out on nodes of the ZIH HPC Capella cluster. Each node is equipped with 
4 x Nvidia H100 GPUs and 2 x AMD EPYC 9334 (32 cores) @ 2.7 GHz CPUs.
% The exact nodes used for each experiment are as follows: 
% P2P Single-node = c87, 
% P2P Multi-node = c87 + c93, 
% Collectives Single-node = c93, 
% Collectives Multi-node = c73 + c118.

Table~\ref{tab:profiler-overhead} shows the average latency per operation (time in \si{\micro\second}) across iterations.
The empty profiler adds roughly \SIrange{8}{9}{\micro\second} overhead per operation in single-node runs (4 GPUs), but introduces negligible overhead in multi-node runs (8 GPUs across 2 nodes).
...
\begin{tabular}{llrr}
\hline
Test & Environment & Without profiler (\si{\micro\second}) & With profiler (\si{\micro\second}) \\
\hline
...
and did not require any changes to the profiler plugin for the used code examples and experiments.

% ---------------------------------------------------------------------------
\section{Discussion}
% ---------------------------------------------------------------------------

This section first discusses practical considerations for developers who implement or extend a NCCL profiler plugin, 
as well as known limitations of the current profiling infrastructure, 
and then shows how the plugin could be integrated with the \mbox{Score-P\xspace} measurement infrastructure for HPC-wide tracing and analysis.

\subsection{Considerations for developers}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"images/ncclgroup stress test/ncclgroup commA 1 collX + commB 1 collY/phase 4, 3rd iter (2).png"}
  \caption{An example illustrating how parentObj and seqNumber can aid in understanding the timing of concurrent collective operations.}
...
Exposing a parent communicator identifier (or equivalent) in the plugin API would enable profilers to reconstruct hierarchical communicator topologies 
and would make for a great addition to future API revisions
...
Integrating NCCL into \mbox{Score-P\xspace} allows developers 
...
NCCL achieves this via \texttt{ncclGetUniqueId}\ncclref{src/init.cc} without a central coordinator. 
...
A \mbox{Score-P\xspace} integration can use these to accurately define process groups. 
...
that translates the \texttt{startEvent} and \texttt{stopEvent} callbacks into \mbox{Score-P\xspace} regions:
The plugin maps NCCL event descriptors (e.g., ncclAllReduce) to \mbox{Score-P\xspace} regions using \mbox{Score-P\xspace} instrumentation
(e.g., \\\texttt{SCOREP\_EnterRegion} and \texttt{SCOREP\_ExitRegion}).

Alternatively, the NCCL profiler plugin can act as a bridge to NVIDIA's Tools Extension (NVTX). 
If \mbox{Score-P\xspace} has been built with CUDA support it can intercept NVTX ranges.
The NCCL profiler plugin would emit \texttt{nvtxRangePush}\cite{nvtx} and \texttt{nvtxRangePop} around NCCL operations. 
\mbox{Score-P\xspace} records these as labeled regions without requiring the plugin to link directly against \mbox{Score-P\xspace} libraries.
This approach decouples the NCCL plugin from the \mbox{Score-P\xspace} build environment
and instead relies on \mbox{Score-P's\xspace} internal NVTX-to-OTF2 mapping logic.

The plugin could also directly utilize \texttt{cuptiActivityPush/PopExternalCorrelationId} to capture GPU activity 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"images/ncclgroup stress test/cupti example (2).png"}
  \caption{CUPTI activity is visualized as orange event bars. 
  With a unique correlation Id, it is possible to trace the activity back to KernelLaunch events}
...
and its suitability for integration with \mbox{Score-P\xspace}.
...
Performance experiments showed that an empty profiler adds roughly \SIrange{8}{9}{\micro\second} overhead per operation in single-node runs 
...
The discussion covered developer considerations, known limitations, and a potential integration strategy with \mbox{Score-P\xspace}.

The NCCL Profiler API allows for highly customized plugins tailored to the analysis needs, 
whether for simple timing, kernel tracing via CUPTI, or integration with external tools such as \mbox{Score-P\xspace}. 
A notable advantage is its low overhead: 
NVIDIA advertises their \texttt{inspector}\ncclref{ext-profiler/inspector} implementation as efficient enough for ``always-on'' profiling in production. 
