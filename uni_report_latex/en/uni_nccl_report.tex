...
Integrating NCCL into this Score-P allows developers 
...
It generates a handle containing 
...
Different MPI tasks or repeated calls in one process each get distinct random magic
and distinct OS-assigned ports are assigned across a cluster, thus collisions are avoided in practice.
Because of this, a Score-P integration can use these to define Process Groups. 
...
A direct integration would potentially involve implementing a NCCL profiler plugin that translates the startEvent and stopEvent callbacks into Score-P regions:
...
The plugin can utilize \texttt{cuptiActivityPushExternalCorrelationId} to capture GPU activity 
