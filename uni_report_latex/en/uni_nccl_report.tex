    ...
    \begin{callself}{nccl}{initTransportsRank}{}
      ...
      \begin{call}{nccl}{ncclProfilerPluginInit}{nccl}{}
        ...
        \begin{callself}{nccl}{ncclProfilerPluginLoad}{}
          ...
          \begin{sdblock}{\texttt{if (profilerPluginStatus != profilerPluginLoadSuccess)}}{}
...
In this section mentioned practical considerations for developers who implement or extend an NCCL profiler plugin, 
outlines known limitations of the current profiling infrastructure, 
and discusses how the plugin could be integrated with the Score-P measurement infrastructure for HPC-wide tracing and analysis.
...
For Score-P, it is important that communicator identities are unique.
NCCL achieves this for \texttt{ncclGetUniqueId}\footnote{\url{https://github.com/NVIDIA/nccl/tree/master/src/init.cc}} without a central coordinator.
Each call fills the bootstrap handle embedded in \texttt{ncclUniqueId} with 
\begin{itemize}
  \item a random 64-bit \texttt{magic} value from \texttt{/dev/urandom}, and 
  \item the socket \texttt{address} of a new listening socket (IP, port), whose port is chosen by the operating system.
\end{itemize}
The pair (random magic, IP+port) is unique in practice: 
different MPI tasks or repeated calls in one process each get distinct random magic and a distinct OS-assigned port so collisions are avoided.

The NCCL profiler plugin is callback-driven \emph{by NCCL}.
...
In one potential integration strategy, a developer would implement the NCCL profiler API 
and could use Score-P's user instrumentation API
(e.g., \texttt{SCOREP\_USER\_REGION\_BY\_NAME\_BEGIN}/\texttt{END})
to inject NCCL Profiler Events as regions into Score-P.
The region name could be derived from the event descriptor (e.g., \texttt{collApi.func} for \texttt{ncclAllReduce}).
...
Alternatively, NVTX and CUPTI could be leveraged with CUDA adapters from Score-P.
Similarly, the NCCL profiler plugin would use the NVTX API\footnote{\url{https://nvidia.github.io/NVTX/doxygen/group___m_a_r_k_e_r_s___a_n_d___r_a_n_g_e_s.html}} to annotate regions.
Additionally, kernel tracing of with CUPTI can be integrated directly into the NCCL profiler plugin.
...
\begin{figure}[H]
  ...
  \caption{CUPTI activity is visualized as orange event bars. Through a correlation Id, it is possible to trace the activity back to \texttt{KernelLaunch} events}
