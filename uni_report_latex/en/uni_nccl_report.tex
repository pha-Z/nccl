...
and \texttt{if (!state->thread)} (omitted in the diagram for readability) 
...
A simplified view of the profiler plugin code used in the examples looks as follows:
...
profiler:
empty - initializes dummy context struct, returns NULL for event handles, tracks all events 4095 (including those for network activity)

application:
syntetic - TODO add application code in an appendix section or something 
